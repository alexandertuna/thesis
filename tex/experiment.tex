...
The L1 trigger is the fastest and most reductive step of the trigger system. Only information from the calorimeter and muon subdetectors are available because the inner detector is not equipped to process events at tens of MHz, and because tracking algorithms are not fast yet fast enough. Performance is sacrificed for speed in the EM calorimeter and muon system. The granularity of the EM calorimeter is reduced for faster processing, as shown in \cref{fig:prospects-trigger-cartoonL1}, and only information from the RPCs and TGCs is read out from the muon system.
